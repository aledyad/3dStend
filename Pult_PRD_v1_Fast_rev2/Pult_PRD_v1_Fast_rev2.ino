...
//---- обработчик нажатия кнопки РАБОТА -----//
  ...
  {  LedRun_HI;// ЛЕД РАБОТА  ГОРИТ
    ...
    if(RunBtn_Read== LOW and RunState==1)
    { LedRun_LO;// ЛЕД РАБОТА  НЕ ГОРИТ
      RunState=0;
      Serial.write("Run OFF\n");
    ZeroState=0;
      delay(500);
  }
...
if (RunState==1) // если режим РАБОТА
  ...
  { ZeroState=1;
    ...
    if(ZeroBtn_Read== LOW and ZeroState==1)
    { ZeroState=0;
...
//---------- вкл. тумблер АВАРИЯ  ----------//
 ...
 {
   if (AlarmBtn_Read == LOW)
     Serial.write("Run OFF ALARM AlarmBtn_Read\n");
   if (!fLink)
     Serial.write("Run OFF ALARM fLink\n");
   if (AlarmStend)
     Serial.write("Run OFF ALARM AlarmStend\n");
 LedAlarm_HI; // ЛЕД АВАРИЯ  ГОРИТ
 AlarmState=1;
 RunState=0;
 Serial.write("Run OFF ALARM\n");
 ZeroState=0;
 LedRun_LO; // ЛЕД РАБОТА  НЕ ГОРИТ
...
// --------- ПЕРЕДАЕМ  ДАННЫЕ НА  СТЕНД  ------------ //
 ...
 // заполняем структуру данными
  ...
  if (SQZ==1) LedZero_HI;//если сработал КВ то горит постоянно
  else
